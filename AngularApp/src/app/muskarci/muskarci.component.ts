...
export class MuskarciComponent implements OnInit {
  ...
  proizvodiSvi: any;
  kolekcijePodaci: any;
  specijalnePonudeMuskarciPodaci: any;
  brojac: number=1;
  ...
  ngOnInit(): void {
    ...
    this.fetchSezone();
    this.fetchKolekcije();
    this.fetchSpecijalnePonudeMuskarci();
    /* setTimeout( ()=>{
...
*/
  ...
  prikaziSpecijalnePonude(){
    if(this.specijalnePonudeMuskarciPodaci==null)
      return null;
    return this.specijalnePonudeMuskarciPodaci.filter((a:any)=>a.proizvod.odjelId==2)
  }

  brojacProvjera(){
    this.brojac++;
    if(this.brojac%2==0){
      this.specijalneMuskarci=true;
    }
    else{
      this.specijalneMuskarci=false;
    }
  }
